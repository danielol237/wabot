...
test("GitHub engineering plugin is public and exposes safe aliases", () => {
  assert.equal(plugin.ownerOnly, false);
  ...
  assert.equal(github.ownerOnly, false);
...
test("GitHub engineering help documents the ordered approval workflow", () => {
  ...
  assert.match(help, /never stores plaintext tokens/i);
});
