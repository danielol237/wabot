...
<script>try{const t=localStorage.getItem('aria-theme');document.documentElement.dataset.theme=t||((matchMedia&&matchMedia('(prefers-color-scheme: dark)').matches)?'dark':'light')}catch(_){document.documentElement.dataset.theme='light'}</script>
<style>
:root{
...
.theme-toggle{border:1px solid var(--line);background:var(--panel);color:var(--muted);border-radius:10px;padding:8px 10px;font:inherit;font-size:12px;font-weight:800;cursor:pointer;white-space:nowrap}.theme-toggle:hover{border-color:var(--accent);color:var(--text)}
:root[data-theme="dark"]{--bg:#0b0d12;--panel:#131720;--panel2:#181d28;--panel3:#202735;--line:#2a3240;--line2:#394354;--text:#f4f6fa;--muted:#9ba6b8;--faint:#697386;--accent:#c9a7ff;--cyan:#73c9e8;--pink:#ff8a9a;--green:#55d5b4;--amber:#f4c96a;--red:#ff8798}:root[data-theme="dark"] body{background:radial-gradient(900px 500px at 92% -8%,#24203a,transparent 62%),radial-gradient(700px 420px at -8% 108%,#102a2a,transparent 58%),var(--bg)}:root[data-theme="dark"] .nav{background:rgba(11,13,18,.9);border-color:var(--line)}:root[data-theme="dark"] .nav .brand span{color:var(--text)}:root[data-theme="dark"] .avatar-mark{background:#171323;box-shadow:0 24px 70px rgba(201,167,255,.2)}:root[data-theme="dark"] .aura{background:radial-gradient(circle,#32274e 0,#1c2739 36%,transparent 70%)}:root[data-theme="dark"] .card,:root[data-theme="dark"] .chat-wrap{background:var(--panel);border-color:var(--line);box-shadow:0 16px 40px rgba(0,0,0,.24)}:root[data-theme="dark"] .chat-input{background:var(--panel2)}:root[data-theme="dark"] .input,:root[data-theme="dark"] .chat-input input{background:var(--surface,var(--panel));color:var(--text);border-color:var(--line)}:root[data-theme="dark"] .msg.aria{background:var(--panel2);color:var(--text);border-color:var(--line)}:root[data-theme="dark"] .mobile-nav{background:rgba(11,13,18,.95);border-color:var(--line)}
</style>
</head>
<body>
<nav class="nav">
  ...
  <button class="theme-toggle" id="theme-toggle" type="button" aria-label="Switch to dark theme">◐ Dark</button>
</nav>
...
function syncTheme(){const dark=document.documentElement.dataset.theme==='dark';themeButton.textContent=dark?'☼ Light':'◐ Dark';themeButton.setAttribute('aria-label',dark?'Switch to light theme':'Switch to dark theme');}
if(themeButton){themeButton.addEventListener('click',()=>{const next=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=next;try{localStorage.setItem('aria-theme',next)}catch(_){}syncTheme()});syncTheme();}
