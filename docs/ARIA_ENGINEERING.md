...
The preferred deployment configuration is still `GITHUB_TOKEN` in the runtime environment for non-user-specific service operations. For multi-user engineering, a user can send a supported GitHub token—or a sentence containing it—to ARIA in their own one-to-one WhatsApp chat. ARIA detects the token before normal AI routing, encrypts and stores it under that sender’s WhatsApp identity, deletes the inbound message on a best-effort basis, never echoes the raw value, and supports `ARIA GitHub status` and `ARIA forget my GitHub token`. A token sent in a group is rejected and the message deletion is attempted; users are told to send it privately. Set `ARIA_CREDENTIAL_ENCRYPTION_KEY` for a stable encryption key across Render replacements. If it is absent, ARIA generates a local key file automatically; this avoids extra configuration but the encrypted credentials are lost if Render replaces the service disk. Neither mode stores plaintext tokens.
