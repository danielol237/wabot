...
function diagnosticState(d) {
  if (d?.candidateCount) return { label: `${d.candidateCount} source${d.candidateCount === 1 ? "" : "s"} found`, tone: "ok" };
  if (d?.noResults) return { label: "No title match", tone: "bad" };
  if (d?.timeout || /timeout|522|5\d\d|ENOTFOUND|migrated|no source|no stream|no servers/i.test(String(d?.error || ""))) return { label: "Temporarily unavailable", tone: "bad" };
  return { label: "No playable source", tone: "bad" };
}

function providerDiagnostics(report) {
  const diagnostics = report?.diagnostics || [];
  const rows = diagnostics.map((d) => { const state = diagnosticState(d); return `<div class="source-row"><span>${esc(providerLabel(d.provider))}</span><span class="source-state ${state.tone}">${esc(state.label)}</span></div>`; }).join("");
  const technical = diagnostics.filter((d) => d.error).map((d) => `${providerLabel(d.provider)}: ${d.error}`).join("\n");
  return rows + (technical ? `<details class="diagnostic-details"><summary>Technical details</summary><pre>${esc(technical)}</pre></details>` : "");
}
...
function animeContext(provider, details = {}, extra = {}) {
  ...
  if (provider === "omnisave" || provider === "curated") {
    ...
    if (details.episodes) params.set("episodes", details.episodes);
    if (details.type) params.set("type", details.type);
...
function titleHref(item) {
  ...
  if (provider === "omnisave" || provider === "curated") {
    ...
    if (item.episodes) params.set("episodes", item.episodes);
...
.fallback-art{position:relative;display:flex;width:100%;height:100%;min-height:100%;flex-direction:column;justify-content:flex-end;gap:4px;overflow:hidden;padding:16px;background:radial-gradient(circle at 76% 18%,var(--art-glow) 0,rgba(255,255,255,0) 28%),linear-gradient(150deg,var(--art-base),#121622);color:#fff}.fallback-art:before{content:"";position:absolute;inset:10% -18% auto auto;width:76%;aspect-ratio:1;border:1px solid rgba(255,255,255,.22);border-radius:50%;box-shadow:0 0 0 28px rgba(255,255,255,.04),0 0 0 58px rgba(255,255,255,.03);transform:rotate(-18deg)}.fallback-art:after{content:"";position:absolute;right:-12%;bottom:-18%;width:72%;aspect-ratio:1;background:linear-gradient(135deg,transparent 35%,var(--art-accent) 36%,transparent 56%);opacity:.48;transform:rotate(18deg)}.fallback-art>*{position:relative;z-index:1}.fallback-art-mark{display:grid;place-items:center;width:42px;height:42px;margin-bottom:auto;border:1px solid rgba(255,255,255,.42);border-radius:14px;background:rgba(10,12,20,.26);font-size:13px;font-weight:900;letter-spacing:.08em}.fallback-art-kicker{font-size:9px;font-weight:900;letter-spacing:.2em;color:var(--art-glow)}.fallback-art strong{max-width:90%;font-size:16px;line-height:1.08;letter-spacing:-.03em}.fallback-art small{font-size:10px;color:rgba(255,255,255,.68)}.fallback-art-hero{padding:40px;justify-content:center;align-items:flex-end;text-align:right}.fallback-art-hero .fallback-art-mark{width:72px;height:72px;margin:0 0 18px;border-radius:22px;font-size:24px}.fallback-art-hero strong{font-size:28px}.fallback-art-hero small{font-size:12px}.fallback-art-poster{padding:14px}.fallback-art-poster .fallback-art-mark{width:54px;height:54px}.poster-fallback{overflow:hidden;padding:0}
@media(max-width:680px){.fallback-art-hero{padding:24px;justify-content:flex-end;align-items:flex-start;text-align:left}.fallback-art-hero strong{font-size:22px}.fallback-art-hero .fallback-art-mark{margin:0 0 auto}.fallback-art-card{padding:12px}.fallback-art-card strong{font-size:13px}.fallback-art-card small{font-size:9px}}
/* ARIA Anime V13 — focused OTT visual system */
:root{--bg:#f7f5fa;--surface:#fff;--surface-2:#f1edf6;--surface-3:#e8e1ef;--line:#e3ddea;--text:#18151d;--muted:#716a7b;--subtle:#9890a4;--accent:#8d63e6;--accent-2:#ef718d;--ok:#2c9b83;--danger:#c9536e;--shadow:0 20px 60px rgba(45,29,69,.12)}
body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;background:radial-gradient(900px 500px at 100% -5%,#eee5fb 0,transparent 68%),radial-gradient(700px 420px at -12% 100%,#e2f4ef 0,transparent 60%),var(--bg)}
.top{background:rgba(247,245,250,.88);backdrop-filter:blur(18px);border-bottom:1px solid rgba(227,221,234,.9)}.top-in{width:min(1280px,100%);padding:14px 28px;gap:24px}.brand-mark{width:40px;height:40px;border-radius:13px;box-shadow:0 8px 24px rgba(141,99,230,.2)}.brand-name{font-size:14px;letter-spacing:.1em}.brand-name b{font-size:10px;letter-spacing:.2em}.nav a,.account-link{font-size:12px;font-weight:800}.search input{min-height:44px;border-radius:13px;background:rgba(255,255,255,.9);border-color:var(--line)}.search button{min-height:44px;border-radius:13px;background:var(--text);color:#fff}.theme-toggle{min-height:40px;border-radius:12px;background:var(--surface);border-color:var(--line)}
.main{width:min(1280px,100%);padding:30px 28px 78px}.hero{min-height:460px;padding:48px;border-radius:28px;border-color:var(--line);background:linear-gradient(135deg,#fff 0%,#f0eafb 100%);box-shadow:var(--shadow)}.hero-art{inset:0 0 0 34%;background:var(--surface-2);overflow:hidden}.hero-art:after{background:linear-gradient(90deg,#fff 0%,rgba(255,255,255,.95) 22%,rgba(255,255,255,.38) 72%,rgba(255,255,255,.04) 100%)}.hero-copy{max-width:620px}.hero h1,.section-title,.quick-start h2,.detail h1,.watch-head h1,.job h1{font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-weight:850;letter-spacing:-.055em}.hero h1{font-size:clamp(36px,5vw,68px);line-height:1.02}.hero p{font-size:15px;color:var(--muted);line-height:1.7}.hero-actions{gap:10px}.btn{min-height:46px;border-radius:13px;font-weight:850}.btn-primary{background:var(--text);color:#fff}.btn-primary:hover{background:var(--accent)}.btn-secondary{background:rgba(255,255,255,.82);border-color:var(--line)}
.section-head{margin:46px 0 17px}.section-title{font-size:28px}.section-title:before{width:5px;height:21px;background:var(--accent-2)}.grid{grid-template-columns:repeat(6,minmax(0,1fr));gap:17px}.card{border-radius:18px;border-color:var(--line);background:rgba(255,255,255,.94);box-shadow:0 8px 25px rgba(45,29,69,.05)}.card:hover{border-color:#cbb7ec;box-shadow:0 17px 40px rgba(141,99,230,.16);transform:translateY(-5px)}.card-body{padding:13px}.card-title{font-size:13px;line-height:1.35}.card-meta{gap:6px}.badge{border-color:var(--line);background:var(--surface-2)}
.artwork{position:relative;overflow:hidden;background:var(--surface-2)}.artwork img{display:block;width:100%;height:100%;object-fit:cover}.artwork-card{aspect-ratio:2/3;border-radius:0}.artwork-hero{position:absolute;inset:0}.artwork-poster{width:100%;aspect-ratio:2/3;border-radius:18px}.artwork-fallback{position:absolute;inset:0}.artwork.is-fallback .artwork-fallback{display:block}.artwork.is-fallback>img{display:none}.artwork-card>.fallback-art{min-height:100%}.artwork-poster>.fallback-art{min-height:100%}.artwork-hero>.fallback-art{min-height:100%;height:100%}.fallback-art{font-family:Inter,ui-sans-serif,system-ui,sans-serif}.fallback-art strong{font-weight:850}
.detail{grid-template-columns:220px minmax(0,1fr);gap:30px;padding:30px;border-radius:24px;background:rgba(255,255,255,.94);border-color:var(--line);box-shadow:var(--shadow)}.episode{border-radius:15px;background:rgba(255,255,255,.9);border-color:var(--line)}.episode-actions .btn{min-height:38px}.quick-start{border-radius:20px;background:linear-gradient(135deg,#fff,#f2edf8);border-color:var(--line)}.official-links,.source-card,.job{border-color:var(--line);background:rgba(255,255,255,.9)}.player-shell{border-radius:20px;box-shadow:var(--shadow)}.empty{border-color:#d7c9e6;background:rgba(255,255,255,.82)}.diagnostic-details{margin-top:14px;text-align:left}.diagnostic-details summary{color:var(--muted);cursor:pointer;font-size:12px;font-weight:800}.diagnostic-details pre{max-width:100%;margin:10px 0 0;overflow:auto;color:var(--subtle);font:11px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap}
:root[data-theme="dark"]{--bg:#090b10;--surface:#141821;--surface-2:#1a202c;--surface-3:#232b39;--line:#2a3445;--text:#f6f7fb;--muted:#aeb7c7;--subtle:#7d889c;--accent:#c7a5ff;--accent-2:#ff839c;--ok:#55d6b5;--danger:#ff879a;--shadow:0 24px 70px rgba(0,0,0,.34)}:root[data-theme="dark"] body{background:radial-gradient(900px 520px at 100% -5%,#211d35 0,transparent 68%),radial-gradient(700px 420px at -12% 100%,#0e2b2b 0,transparent 60%),var(--bg)}:root[data-theme="dark"] .top{background:rgba(9,11,16,.88);border-color:rgba(42,52,69,.9)}:root[data-theme="dark"] .hero{background:linear-gradient(135deg,#141821 0%,#201a31 100%);border-color:var(--line)}:root[data-theme="dark"] .hero-art:after{background:linear-gradient(90deg,#141821 0%,rgba(20,24,33,.96) 22%,rgba(20,24,33,.38) 72%,rgba(20,24,33,.04) 100%)}:root[data-theme="dark"] .search input,:root[data-theme="dark"] .theme-toggle,:root[data-theme="dark"] .account-link{background:var(--surface);color:var(--text);border-color:var(--line)}:root[data-theme="dark"] .search button,:root[data-theme="dark"] .btn-primary{background:var(--accent);color:#1b1227}:root[data-theme="dark"] .btn-secondary{background:var(--surface-2);border-color:var(--line);color:var(--text)}:root[data-theme="dark"] .card,:root[data-theme="dark"] .detail,:root[data-theme="dark"] .episode,:root[data-theme="dark"] .job,:root[data-theme="dark"] .official-links,:root[data-theme="dark"] .source-card{background:rgba(20,24,33,.94);border-color:var(--line)}:root[data-theme="dark"] .quick-start{background:linear-gradient(135deg,#141821,#201a31);border-color:var(--line)}:root[data-theme="dark"] .empty{background:rgba(20,24,33,.94);border-color:var(--line)}
@media(max-width:1050px){.grid{grid-template-columns:repeat(4,minmax(0,1fr))}}
@media(max-width:680px){.top-in{grid-template-columns:minmax(0,1fr) auto;padding:11px 14px;gap:10px}.main{padding:18px 14px 54px}.hero{min-height:450px;padding:24px;border-radius:20px}.hero-art{inset:0 0 47% 0}.hero-art:after{background:linear-gradient(0deg,#fff 7%,rgba(255,255,255,.82) 57%,rgba(255,255,255,.04) 100%)}:root[data-theme="dark"] .hero-art:after{background:linear-gradient(0deg,#141821 7%,rgba(20,24,33,.84) 57%,rgba(20,24,33,.04) 100%)}.hero-copy{z-index:1}.hero h1{font-size:clamp(34px,10vw,48px)}.hero-actions{display:grid;grid-template-columns:1fr 1fr}.hero-actions .btn{width:100%}.grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}.card{border-radius:15px}.card-body{padding:10px}.card-title{font-size:12px}.detail{grid-template-columns:96px minmax(0,1fr);gap:16px;padding:18px;border-radius:19px}.detail>.artwork{grid-column:1}.detail>.artwork+.artwork-fallback{}.detail .desc,.detail .official-links,.detail .detail-actions{grid-column:1/-1}.section-title{font-size:24px}.quick-start{grid-template-columns:1fr;padding:18px}.quick-actions{display:grid;grid-template-columns:1fr}.official-links{grid-template-columns:1fr}.source-state{max-width:58%}.episode-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}}
@media(max-width:380px){.hero-actions{grid-template-columns:1fr}.hero{min-height:420px;padding:20px}.detail{grid-template-columns:82px minmax(0,1fr)}.grid{gap:9px}}
</style></head><body>
...
const FALLBACK_ART_PALETTES = [
  ["#201a3d", "#8e6bd6", "#f5b6ff"],
  ["#102b36", "#26b3b0", "#b4fff0"],
  ["#321b2a", "#ed7184", "#ffd0b8"],
  ["#18253c", "#4a8dff", "#c9ddff"],
];

function fallbackArtwork(title, variant = "card") {
  const text = String(title || "ARIA Anime");
  const hash = [...text].reduce((total, char) => total + char.charCodeAt(0), 0);
  const [base, accent, glow] = FALLBACK_ART_PALETTES[hash % FALLBACK_ART_PALETTES.length];
  const initials = text.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "AA";
  return `<div class="fallback-art fallback-art-${esc(variant)}" style="--art-base:${base};--art-accent:${accent};--art-glow:${glow}"><span class="fallback-art-mark">${esc(initials)}</span><span class="fallback-art-kicker">ARIA ANIME</span><strong>${esc(text)}</strong><small>Source-ready metadata</small></div>`;
}

function artwork(item, variant = "card") {
  const title = item?.title || "ARIA Anime";
  const fallback = fallbackArtwork(title, variant);
  const cover = String(item?.cover || "").trim();
  if (!cover) return `<div class="artwork artwork-${esc(variant)} is-fallback">${fallback}</div>`;
  return `<div class="artwork artwork-${esc(variant)}"><img src="${esc(cover)}" alt="${esc(title)} poster" loading="${variant === "hero" ? "eager" : "lazy"}" onerror="this.hidden=true;this.nextElementSibling.hidden=false;this.closest('.artwork')?.classList.add('is-fallback')"><div class="artwork-fallback" hidden>${fallback}</div></div>`;
}
...
async function homePage() {
  ...
  const heroHtml = hero ? `<section class="hero"><div class="hero-art">${artwork(hero, "hero")}</div><div class="hero-copy"><div class="eyebrow">Featured this week · ${esc(providerLabel(hero.provider))}</div><h1>${esc(hero.title)}</h1><p>${esc(hero.overview || hero.description || "Open a title to see available episodes and authorized media options.")}</p><div class="hero-meta"><span class="meta-pill score">${hero.rating ? `★ ${esc(hero.rating)}` : "Featured"}</span>${hero.year ? `<span class="meta-pill">${esc(hero.year)}</span>` : ""}${hero.type ? `<span class="meta-pill">${esc(hero.type)}</span>` : ""}</div><div class="hero-actions">${button(titleHref(hero), "Open title", "primary")}${button("/anime/browse", "Browse catalog", "secondary")}</div></div></section>` : `<section class="hero"><div class="hero-art">${artwork({ title: "ARIA Anime" }, "hero")}</div><div class="hero-copy"><div class="eyebrow">ARIA Anime</div><h1>Find your next series.</h1><p>Search the catalog or browse the latest metadata.</p></div></section>`;
...
async function titlePage(req) {
  ...
  const entry = {
    ...
    episodes: Number(req.query.episodes) || 0,
