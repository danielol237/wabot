...
<a class="brand" href="/anime" aria-label="ARIA Anime home"><span class="brand-mark" aria-hidden="true">A</span><span class="brand-name">ARIA <b>ANIME</b></span></a>
...
async function titlePage(req) {
  ...
  if (details.isAdult || details.blocked) return layout("Title unavailable", `<div class="empty"><h1>Title unavailable</h1><p>This title is not included in the public catalog.</p>${button("/anime", "Back to home", "primary")}</div>`);
...
async function watchPage(req) {
  ...
  const mediaUrl = `/anime/proxy?u=${encodeURIComponent(source.url)}&r=${encodeURIComponent(source.headers?.Referer || source.headers?.referer || "")}`;
...
async function downloadPage(req, res) {
  ...
  const result = fileReady ? `<div class="download-actions">${button(`/anime/file/${encodeURIComponent(job.id)}`, "Download file", "primary")}${button(`/anime/title/${encodeURIComponent(id)}?prov=${encodeURIComponent(provider)}`, "Back to episodes", "secondary")}</div>` : "";
...
router.get("/proxy", (req, res) => {
  ...
  const referer = String(req.query.r || "");
  if (!/^https?:\/\//i.test(target)) return res.status(400).send("Invalid media URL.");
  let parsed;
  try { parsed = new URL(target); } catch (_) { return res.status(400).send("Invalid media URL."); }
  const lib = parsed.protocol === "https:" ? https : http;
  const headers = { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/131 Safari/537.36" };
  if (referer) { headers.Referer = referer; headers.Origin = (() => { try { return new URL(referer).origin; } catch (_) { return ""; } })(); }
  if (req.headers.range) headers.Range = req.headers.range;
  const upstream = lib.get(target, { headers }, (response) => {
  ...
  upstream.on("error", (error) => { if (!res.headersSent) res.status(502).send(`Media proxy error: ${esc(error.message)}`); else res.end(); });
});
...
router.get("/dl/:id", async (req, res) => { try { const page = await downloadPage(req, res); if (!res.headersSent && page) res.send(page); } catch (e) { if (!res.headersSent) res.status(500).send(esc(e.message)); } });
