...
<script>try{const t=localStorage.getItem('aria-academy-theme');document.documentElement.dataset.theme=t||((matchMedia&&matchMedia('(prefers-color-scheme:dark)').matches)?'dark':'light')}catch(_){document.documentElement.dataset.theme='light'}</script>
<style>
...
.theme-toggle{position:fixed;z-index:30;top:18px;right:20px;border:1px solid var(--line);background:var(--panel);color:var(--muted);border-radius:11px;padding:9px 11px;font:inherit;font-size:12px;font-weight:800;cursor:pointer;box-shadow:0 8px 20px rgba(55,36,76,.08)}.theme-toggle:hover{border-color:var(--accent);color:var(--text)}:root[data-theme="dark"]{--bg:#0c0e13;--panel:#141821;--panel2:#1a202b;--line:#2b3544;--text:#f6f7fb;--muted:#aeb7c7;--faint:#7f8a9d;--accent:#c9a7ff;--accent2:#ff8b9a;--danger:#ff8798;--shadow:0 20px 60px rgba(0,0,0,.32)}:root[data-theme="dark"] body{background:radial-gradient(850px 480px at 90% 0,#222036,transparent 64%),radial-gradient(700px 400px at -10% 90%,#102a2a,transparent 58%),var(--bg)}:root[data-theme="dark"] .auth-layout,:root[data-theme="dark"] .portal-card,:root[data-theme="dark"] .metric,:root[data-theme="dark"] .leader-row{background:var(--panel);border-color:var(--line);box-shadow:0 16px 40px rgba(0,0,0,.22)}:root[data-theme="dark"] .auth-aside{background:linear-gradient(155deg,#1e1a2d,#141821 74%);border-color:var(--line)}:root[data-theme="dark"] .auth-form{background:#11151d}:root[data-theme="dark"] .brand-mark{background:#1b1625}:root[data-theme="dark"] .field input,:root[data-theme="dark"] .link-input{background:var(--panel2);color:var(--text);border-color:var(--line)}:root[data-theme="dark"] .gbtn{background:#f7f8fb;color:#151821}:root[data-theme="dark"] .btn{background:var(--accent);color:#1d1325}:root[data-theme="dark"] .btn.primary{background:var(--accent2);color:#24131a}:root[data-theme="dark"] .metric{background:linear-gradient(145deg,#141821,#1e1a2d)}:root[data-theme="dark"] .notice{background:#172b2a;border-color:#28514c}:root[data-theme="dark"] .tag{background:#252038;border-color:#4b3d6a;color:var(--accent)}@media(max-width:760px){.theme-toggle{top:12px;right:12px;padding:8px 9px}.theme-toggle .theme-label{display:none}}
</style></head><body><button class="theme-toggle" id="theme-toggle" type="button" aria-label="Switch to dark theme"><span class="theme-icon">◐</span> <span class="theme-label">Dark</span></button>${body}<script>(function(){const b=document.getElementById('theme-toggle');if(!b)return;function sync(){const d=document.documentElement.dataset.theme==='dark';b.querySelector('.theme-icon').textContent=d?'☼':'◐';b.querySelector('.theme-label').textContent=d?'Light':'Dark';b.setAttribute('aria-label',d?'Switch to light theme':'Switch to dark theme')}b.addEventListener('click',function(){const n=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=n;try{localStorage.setItem('aria-academy-theme',n)}catch(_){}sync()});sync()})()</script></body></html>`;
